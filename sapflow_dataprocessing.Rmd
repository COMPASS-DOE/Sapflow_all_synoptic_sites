...
#reading L2 dataset
#synoptic_full <- readRDS("C:/Users/rbitten/Downloads/synoptic_full.rds")
...
sapflow_dtmax <- sapflow %>%
  ...
  dplyr::select(TIMESTAMP, DATE, hour, Site, Plot, Sensor_ID,`sapflow-3.5cm` ) %>%
  filter(hour >= 0 & hour <= 4,        # night
        "sapflow-3.5cm" > 0.2 ,        # bad data - maybe low V ???
        "wx-par-den15" > 200          # night
                                       # TODO: include here the vpd filter
        ) %>%    
