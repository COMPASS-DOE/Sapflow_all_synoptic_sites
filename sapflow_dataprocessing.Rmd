...
# night:        filter(Hour >= 23, Hour <= 4)
# Low PAR:      filter (PAR < 200)
# Low VPD:      filter (VPD < 0.2)
# VPD stability:  4 points <2SD from what??
...
WINDOW_WIDTH <- 14 * 24 * 4 # 14 days times 24 hours per day times 4 obs per hours (15min interval)
sapflow_df$VPD_rollmean <- zoo::rollmean(sapflow_df$`wx-vpd15`, k = 14 * 24 * 4, fill = "extend", na.rm = TRUE)
sapflow_df$VPD_rollsd <- rollapply(sapflow_df$`wx-vpd15`, width = WINDOW_WIDTH, FUN = sd, partial = TRUE, na.rm = TRUE )
...
# Compute "day of night" -- 23h-24h should be used for the following day's
# computation, i.e. grouped with 1h-4h of the next day
sapflow_df$day_of_night <- if_else(sapflow_df$hour >= 23, sapflow_df$day + 1, sapflow_df$day)

night_stats<- sapflow_df %>% 
  filter(hour >= 23 | hour <=4) %>% 
  group_by(Sensor_ID, day_of_night) %>% 
  mutate(accept = (!is.na(`wx-par-den15`) & `wx-par-den15` < 200) &
            (!is.na(`wx-vpd15`)     & `wx-vpd15` < 0.2) &
            (!is.na(VPD_rollmean)   & !is.na(VPD_rollsd) &
                # within 2 standard deviations of 14-day mean
                (`wx-vpd15` - VPD_rollmean) < 2 * VPD_rollsd),
        dTmax = if_else(
            any(accept, na.rm = TRUE),                          # is there any accepted line?
            max(`sapflow-3.5cm`[accept == TRUE], na.rm = TRUE), # max only in the accepted lines
            NA_real_                                            # otherwise NA (not -Inf)
        )
    ) %>%
  ungroup() %>%
  # Now fill the NAs (values fill down until next non-NA value)
          tidyr::fill(dTmax, .direction = "downup")
...
night_stats %>% 
  mutate(F = 0.011899 * (((dTmax / `sapflow-3.5cm`) - 1)^1.231)) -> sf_data
...
sf_plot_avg_june <- compute_avg_sapflow(sf_data, "2022-06-12")
sf_plot_avg_july <- compute_avg_sapflow(sf_data, "2022-07-06")


p_june <- ggplot(sf_plot_avg_june) + 
  geom_point(aes(x = Hour, y = F_avg, color = Plot)) + 
  geom_line(aes(x = Hour, y = F_avg, color = Plot)) +
  labs(y = "Sap Flow L/h", x = "Hour", title = "LITU, June 12th 2022") +
  theme_minimal()

p_july <- ggplot(sf_plot_avg_july) + 
  geom_point(aes(x = Hour, y = F_avg, color = Plot)) +
  geom_line(aes(x = Hour, y = F_avg, color = Plot)) +
  labs(y = "Sap Flow L/h", x = "Hour", title = "LITU, July 6th 2022") +
  theme_minimal()
